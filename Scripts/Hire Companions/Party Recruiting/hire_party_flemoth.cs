//---------------------------------------------------------------------
/*
     Adds Flemeth to the party by tag, without location restriction

     usage:

        runscript hire_party_flemoth or talk to npc to hire her.

     Note:

        1. This file overwrite the original file "hire_party_flemoth.nss" to
        fix old problem.
...
int SpawnFollower(){
    ...
    object oCreature= GetObjectByTag(GEN_FL_Flemeth);
    int FollowerState = 0;
    int result = TRUE;
    ...
    if(!IsObjectValid(oCreature)){
       oCreature = CreateObject(OBJECT_TYPE_CREATURE, R"party_flemoth.utc", GetLocation(OBJECT_SELF));
    }
    ...
    WR_SetPlotFlag(PLT_GEN00PT_PARTY_RECRUIT, GEN_FLEMETH_RECRUITED, TRUE);
