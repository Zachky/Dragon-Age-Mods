...
int SpawnFollower(){
    ...
    if(!IsObjectValid(oCreature)){
       oCreature = CreateObject(OBJECT_TYPE_CREATURE, R_Martin, GetLocation(OBJECT_SELF));
    }
    ...
    //Set flag "Is_not_virgin" to true if player conscript Martin at mod's map.
    //When player fire Martin, he will respawn at Recruit Center instead.
    if(MapTag == TW_riverton){
       WR_SetPlotFlag(PLT_GEN00PT_PARTY_MARTIN, GEN_MARTIN_IS_NOT_VIRGIN, TRUE);
    }
