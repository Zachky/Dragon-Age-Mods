...
int SpawnFollower(){
    ...
    if(!IsModInstall(Small_Restoration)){return FALSE;}

    //Create follower next to Warden if follower does not exist
    if(!IsObjectValid(oCreature)){
       oCreature = CreateObject(OBJECT_TYPE_CREATURE, R_Moira, GetLocation(OBJECT_SELF));
    }
...
void main()
{
    ...
    if(Result){
       ...
       DisplayFloatyMessage(oMainControlFollower, Msg_SR, FLOATY_MESSAGE, 0xff0000, 2.0);
    }
