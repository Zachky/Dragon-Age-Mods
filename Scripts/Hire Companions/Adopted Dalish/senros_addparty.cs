//---------------------------------------------------------------------
/*
     Adds Senros to the party by tag, without location restriction

     usage:

        runscript senros_addparty
...
void main()
{
    ...
    if(oCreature != OBJECT_INVALID){
        ...
        if(!IsObjectValid(oCreature)){
           oCreature = CreateObject(OBJECT_TYPE_CREATURE, R"ado_companion_senros.utc", GetLocation(OBJECT_SELF));
        }

        //Set plot flag "Recruited" to true for other feature
        WR_SetPlotFlag(PLT_GEN00PT_ADOPTED_DALISH, GEN_SENROS_RECRUITED, TRUE);
