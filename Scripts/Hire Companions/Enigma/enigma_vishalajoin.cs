//---------------------------------------------------------------------
/*
     Adds Vishala to the party by tag, without location restriction

     usage:

        runscript vishala_addparty
...
void main()
{
    ...
    if(oCreature != OBJECT_INVALID){
        ...
        if(!IsObjectValid(oCreature)){
           oCreature = CreateObject(OBJECT_TYPE_CREATURE, R"enigma_hero1.utc", GetLocation(OBJECT_SELF));
        }

        //Set plot flag "Recruited" to true for other feature
        WR_SetPlotFlag(PLT_ENIGMA_PLOT1, GEN_VISHALA_RECRUITED, TRUE);
