//---------------------------------------------------------------------
/*
     Adds Ariane to the party by tag, without location restriction

     usage:

        runscript ariane_addparty
...
int SpawnFollower(){
    ...
    if(!IsObjectValid(oCreature)){
       oCreature = CreateObject(OBJECT_TYPE_CREATURE, R_Ariane, GetLocation(OBJECT_SELF));
    }
    ...
    WR_SetPlotFlag(PLT_GEN00PT_RETURN_TO_KW, GEN_ARIANE_RECRUITED, TRUE);

    //Set this flag true if player conscript ariane at mod's map.
    //When player fire Ariane, she will respawn at Recruit Center instead.
