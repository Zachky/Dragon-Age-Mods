/*
     Remove Sanros from party

     usage:

        runscript sanros_fire
...
void main()
{
   object oFollower = GetObjectByTag(GEN_FL_Senros);
   ...
   WR_SetPlotFlag(PLT_GEN00PT_ADOPTED_DALISH, GEN_SENROS_RECRUITED, FALSE);

   DestroyObject(oFollower);

   //Respawn the npc if warden sit on the map where npc show up in the first place.
   if(MapTag == CORE_RecruitCenter){
      SpawnCompanion_location(Adopted_Dalish, MapTag, GEN_FL_Senros, R_Senros);
   }
